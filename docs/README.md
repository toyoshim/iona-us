...
## 注意
以下の説明ではバージョンに応じた説明の分かれる個所がありますが、バージョンには基板のバージョンとファームウェアのバージョンがあります。v1基板、v2基板は全てのファームウェアが動作しますが、v3基板はファームウェア1.48または2.11以降しか動作しません。
...
naomiの電脳戦機バーチャロン オラトリオ・タングラムの特殊レイアウトやスーチーパイIII等の麻雀レイアウト、レースやガンコントローラにも対応可能です。
...
⑤には別のJVS I/Oデバイスを接続できます。

電源投入時にはLEDが高速点滅し、JVSバスからアドレスを設定され通信可能となったタイミングで点灯に変化します。JVSバスからリセット信号を受けた際には再度高速点滅を始めます。

## 機能 (v2.xx系ファームウェア)
以下の説明は最新バージョンについての説明です。
古いバージョンでは一部利用できない機能もあるのでご注意ください。

### 設定編集
[設定ページ](setting)で6種類の設定を自由に編集し、IONA-US単体では保存した6種類の設定を切り替えて使用します。各設定は、以下のような情報を持ちます。
- ホストに対して名乗るデバイス名
- ホストに対してサポートしていると宣言する機能の詳細（アナログ入力数や入力ビット幅など）
- 各ボタンを押した際に、どのボタンの組み合わせとしてホストにレポートするかのレイアウト情報
- 各ボタンに対する連射設定の割り付け
- 連射設定（パタンと周期）
- アナログスティックやアナログトリガーの割り当て（レバーまたは各種アナログ入力）

### 設定切り替え
TESTとSERVICEを1秒ほど長押しするとLEDが消えて設定切り替えモードに入ります。
この状態でTESTボタンを押すことで、設定番号が1進み、SERVICEボタンで確定して通常モードへ戻ります。
TESTを押した際のLEDの点滅回数で現在の設定番号がわかります。

電源投入後にLEDの３回点滅が続く場合、設定データが初期化されていないか壊れている可能性がありますので、[設定ページ](setting)からv2.xx向けの設定を上書き保存する事で初期化して下さい。v1.xxから移行する際には必須です。

### ガンコン設定
TSS-I/Oモードで動作する基板の中にはガンコンのキャリブレーションができないタイトルがあります。
その場合、TESTとSERVICEを3秒ほど長押しする事で消えたLEDが再び点滅を始め、キャリブレーションモードに入ります。
順番は問いませんので画面の四隅を撃つ事でキャリブレーションが終了します。
キャリブレーションデータと選択中の設定は内蔵フラッシュに保存されます。
他の設定とは別の場所に保存されており、ファームウェア更新時にのみキャリブレーションデータは初期化されます。
...
|-|-|-|-|-|OFF|-|-|1.41|アナログ0,1をレバーにも割り当てる(*4)|
|-|-|-|-|-|ON|-|-|1.41|アナログ0,1をレバーには割り当てない(*4)|
|-|-|-|-|-|-|OFF|-|1.45|アナログ入力をサポートしない|
|-|-|-|-|-|-|ON|-|1.45|アナログ入力をサポートする|
|-|-|-|-|-|-|-|OFF|1.46|JVS Dash(*5)をサポートする|
|-|-|-|-|-|-|-|ON|1.46|JVS Dash(*5)をサポートしない|
...
(*4) 1.44まで設定が反対になっていました。

(*5) exA-Arcadiaで最近追加された高速モード、既知の対応タイトルは東方電幻景のみ。
...
## バージョンについて
### 基板のバージョン
- 過去にVer 1.10、1.20、2.00が出荷され、今後は3.11のみ出荷されます。

#### Ver 1.10基板固有の問題点
namco系のボードから認識されない場合に設定で回避できませんでした。
対応するための[改造](mod)を紹介しています。
トライフォースやSystem 2x6などが該当するようです。
naomiの一部のロットも該当する事があるようです。

#### Ver 1.20、2.00基板固有の問題点
標準のnamco系に対応した動作モードでは、一部のnaomi個体から認識されにくくなる報告があります。
もし __naomiやSEGA系の基板との接続がうまくいかない場合は、オプション設定でJVSデータ信号レベル補正を無効にて下さい。__
また __exA-Arcadiaにおいても本設定を無効にしないと、システムでは認識されてもJVS Dash対応タイトル起動時にI/O Errorが出る事が確認されています。__
1.xx系の新しいファームウェアではTESTボタンを押しながら電源を入れることで設定が反転します（設定変更は保持されます）のでご確認下さい。

### ファームウェアのバージョン
- 最新版ではVer 2.16が初期状態で書き込まれています。バージョンはデバイス名として確認可能です。
- ファームウェアは[ファームウェア](firmware)ページから任意のバージョンに更新可能です（ただしv3系基板ではv1.48とv.211以降のみ動作）。
- Ver 2.00以降は[設定ページ](setting)から設定できるようになり、より詳細なカスタマイズが可能です。

#### Ver 1.3x系固有の説明
P1のアナログスティックX/Yをアナログ0,1に、P2のアナログスティックX/Yを2,3に割り当ててあります。
Ver 1.31以降はガンコンモードが加わりました。

#### Ver 1.4x系固有の説明
P1の左アナログスティックX/Yをアナログ0,1に、右アナログスティックX/Yをアナログ2,3に、LT/RTがアナログ対応の場合はアナログ4,5に割り当ててありますが、アナログレイアウトで設定変更可能です。設定変更可能になったことからガンコンモードは廃止しました。

## おまけ情報
### 固定用の穴について
v2/v3系基板では固定用のネジ穴があります。基板の形状データは以下のとおりですので参考にして下さい。
ネジ穴の大きさはΦ3.5になります。
![foot_print](foot_print.png)

### ケースについて
Ver 1.xx基板、Ver 2.00基板それぞれについてケースを自作してデータを公開しているユーザーがいるので紹介させてください。Ver 3.xx基板は2.00基板と基本的には同じレイアウトです。
...
- [「MP07-IONA-US」Ver 2.xx用ケース（STLデータ）](https://pomegd.booth.pm/items/4558497) for Ver 2.00 by ぽめ
...
v3系基板では追加のICで入出力を受けることで、EIA-RS485のフルレンジに対応し、この互換性問題は発生しなくなりました。

## 問い合わせ
質問は[Twitter](https://twitter.com/toyoshim)等で声をかけて頂ければできる限りお答えします。
相性問題の報告やUSBコントローラの対応要望、機能追加のアイデアどもあればぜひご連絡ください。

関連情報はハッシュタグ[#IONA_US](https://twitter.com/hashtag/IONA_US)にて発信します。感想などもこのハッシュタグをご利用下さい。